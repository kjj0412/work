...
def Broad_RePurchase(df, Brand):
    ...
    * Cohort_Days : 재구매 유저의 구매일자 - 첫구매일자
    ...
    Order_df['Broad_Repurchase_User'] = Order_df.apply(lambda x : x['Broad_Repurchase_User_DB'] if x['Last_Sequence'] > 0 else x['Broad_Repurchase_User_new'], axis=1) #여기까지봄
...
def main(Brand, start, end, update_all):
    ...
    * SKU 매핑하며 left outer join이 되기 때문에 Quantity, Sales는 행분리된 개수를 카운팅해서 다시 나눠줌 (Quantity_divide, Sales_divide)
    ...
    if Brd == 'fs':
        Option_df.columns = ['idx', '주문상품명', '상품옵션', 'SKU', 'Quantity_Bundle', 'Set', 'Item', 'Shape', 'Lineup', 'Collection']
    else:
